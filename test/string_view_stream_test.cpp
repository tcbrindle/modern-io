...
static_assert(io::is_sync_read_stream_v<io::string_view_stream>,
              "string_view_stream does not meet the SyncReadStream requirements");
static_assert(io::is_seekable_stream_v<io::string_view_stream>,
...
TEST_CASE("string_view streams can be read from", "[string_view_stream]")
{
    ...
    SECTION("...using a dynamic buffer") {
        ...
        REQUIRE(std::equal(std::cbegin(buf), std::cend(buf),
                           std::cbegin(test_string_view), std::cend(test_string_view)));
    }
